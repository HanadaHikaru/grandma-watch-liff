...
const liffRelease=Object.freeze({version:"1.0.0-rc.1",revision:"300882e",builtAt:"2026-08-17T06:41:17.920Z"});
...
async function statusView(){
  ...
  app.innerHTML=`<div class="subject-name">${escapeHtml(value.subjectDisplayName)}さんの見守り</div><section class="card status-card status-${escapeHtml(value.status).toLowerCase()}"><div class="state">${escapeHtml(stateName(value.status))}</div><div class="row"><span>最終通信</span><strong>${format(value.lastCommunicationAt)}</strong></div><div class="row"><span>最終位置取得</span><strong>${format(value.lastLocationAt)}</strong></div><div class="row"><span>最終スマホ操作</span><strong>${value.interactionAvailable===true?format(value.lastUnlockedAt):"端末側で要設定"}</strong></div><div class="row"><span>現在地</span><strong>${escapeHtml(value.locationLabel)}</strong></div><div class="row"><span>バッテリー</span><strong>${value.batteryPercent==null?"未取得":`${value.batteryPercent}%`}</strong></div><div class="row"><span>充電</span><strong>${value.charging==null?"未取得":value.charging?"充電中":"充電していません"}</strong></div>${actions?`<div class="actions two-actions">${actions}</div>`:""}</section>${alertCards(alerts.items)}${management}`;
...
async function systemView(){const [value,device]=await Promise.all([api(config.adminApiUrl,"system-status"),api(config.adminApiUrl,"device-status")]);subtitle.textContent="管理・設定";const deviceHtml=device.enrolled?`<section class="card"><h2>BASIO端末</h2><div class="row"><span>機種</span><strong>${escapeHtml(device.deviceModel)}</strong></div><div class="row"><span>Android</span><strong>${escapeHtml(device.androidVersion)}</strong></div><div class="row"><span>BASIOアプリ版</span><strong>${escapeHtml(device.lastReportAppVersion??device.enrolledAppVersion)}</strong></div><div class="row"><span>使用状況アクセス</span><strong>${device.interactionAvailable?"許可済み":"端末側で要設定"}</strong></div><div class="row"><span>最終スマホ操作</span><strong>${format(device.lastUnlockedAt)}</strong></div><div class="row"><span>最終認証</span><strong>${format(device.lastAuthenticatedAt)}</strong></div><div class="row"><span>最終通信</span><strong>${format(device.lastCommunicationAt)}</strong></div><div class="row"><span>最終位置</span><strong>${format(device.lastLocationAt)}</strong></div><div class="row"><span>最終レポート時の通信</span><strong>${device.networkAvailable==null?"不明":device.networkAvailable?"利用可":"利用不可"}</strong></div><div class="row"><span>稼働</span><strong>${device.active?"有効":"停止"}</strong></div></section>`:'<section class="card"><h2>BASIO端末</h2><p>登録済み端末はありません。</p></section>';const releaseHtml=`<section class="card"><h2>家族画面</h2><div class="row"><span>バージョン</span><strong>${escapeHtml(liffRelease.version)}</strong></div><div class="row"><span>ビルドID</span><strong>${escapeHtml(releaseRevision())}</strong></div><div class="row"><span>公開日時</span><strong>${escapeHtml(releaseBuiltAt())}</strong></div><p class="muted">表示が古い場合は、画面を閉じてLINEから開き直してください。</p></section>`;app.innerHTML=`${deviceHtml}${releaseHtml}<section class="card"><h2>連携状態</h2>${value.items.map(item=>`<div class="row"><span>${escapeHtml(item.component)}</span><strong>${escapeHtml(item.status)}</strong></div>`).join("")||"状態記録なし"}</section>`;}
